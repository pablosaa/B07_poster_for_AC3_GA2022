...
\begin{poster}%
  ...
  % Authors
  {\color{blue} Pablo Saavedra~Garfias, and Heike Kalesse-Los, and Luisa von~Adebyll} %, and Hannes Griesche$^{3}$, and Gunnar Spren$^{4}$\\
    ...
    %$^3$Alfred Wegener Institute, Helmholtz Centre for Polar and Marine Research AWI\\
    %$^4$Leipzig Institute for Tropospheric Research TROPOS\\
    %$^5$Institute of Environmental Physics, University of Bremen
...
\begin{tabular*}{0.99\textwidth}[h!]{lr}
	\begin{minipage}{0.7\textwidth}
		\vspace{-4em}
	{\small Main interesst are leg 1 to 3 of the \mosaic expedition~\ref{bib:Shupe2022}. Instrumention and data set are mainly provided by the Atmospheric Radiation Measurement’s (ARM) Mobile Facility 1 (MF-1) and the OCEANET-Atmosphere container from TROPOS.}
	\end{minipage}
	&
	\includegraphics[scale=0.25]{arctic_Pstern_drift20192020.png}	
\end{tabular*}
\\
The case study of 18th November 2019 is used to highlight the reseach methods.
  }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\headerbox{2.- Coupling Sea Ice and Clouds}{name=methods,column=0,below=objective}{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\hspace{-2em}
Daily sea ice lead fraction (LF) is obtained from space-borne observations based on the divergence calculations from consecutive Sentinel-1 SAR scenes~\ref{bib:vonAlbedyll2021}. Additionally sea ice concentration (SIC) is also provided by the University of Bremen~\ref{bib:Ludwig2021}. Fig.~\ref{fig:lf_sic} summarizes the LF and SIC during the period of interest.
...
The following analysis is performed to couple sea ice lead fraction with cloud observations above \polarstern:

\colouredcircle LF products is analyzed for a sector 50~km around the \polarstern (red star in Fig.~\ref{fig:lf_sic}, right), with update coordinates every minute.\\

\colouredcircle Sea ice - atmosphere coupling conceptual model\\
Vertical gradient of water vapour transport ($\nabla WVT$) is calculated from humidity $q_v$ and horizontal wind $\vec{v}_w$ radiosonde profiles, following
\begin{equation}
...
in K, $\Delta \theta_v\,=\,\theta_v-\theta_v(z_0)$, $\Delta u\,=\,u-u_0$, and $\Delta v\,=\,v-v_0$, the horizontal wind components in $\rm m~s^{-1}$. The $\Delta z\,=\,z - z_0$ with $z$ the altitude of the atmosphere layers in $\rm m$ and the subscript $0$ indicates the surface reference.
