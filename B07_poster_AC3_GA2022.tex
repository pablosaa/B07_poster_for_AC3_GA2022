...
\begin{tabular}{cc} %{@{}lcr@{}}
	...
	\colouredcircle {\larger Cloud base and -top height (CBH and CTH, respectively) are additionally determined from the Cloudnet target classification class (Fig.~\ref{fig:classification}, top) for up to three cloud layers.} To estimate CBH the Cloudnet pixels with liquid droplets or super-cooled liquid (SCL) are considered, for CTH the ice pixels are used in additon (Fig.~\ref{fig:closeup}).\\
\begin{tabular}{ccc}
	%\textbf{\Large COUPLED}	& \textbf{\Large DECOUPLED} \\
