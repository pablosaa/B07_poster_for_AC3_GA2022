...
\begin{equation}
	\Gamma_{\rm cloud}=\frac{\Delta T}{\Delta H}\,=\,\frac{\Delta T=T_{top}-T_{base}}{CTH-CBH}
\end{equation}
 (with $\Delta T=T_{top}-T_{base}$ and $\Delta H=CTH-CBH$ . The most negative $\Gamma_{\rm cloud}$ are closer to a moist adiabatic lapse-rates, whereas positive values indicates inversion at cloud top, however this is not consistent neither with increasing LF nor thickness since for coupled cases there are positive and negative $\Gamma_{\rm cloud}$ which is an indicator of other factors affecting it.
